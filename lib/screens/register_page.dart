...
class RegisterPage extends StatefulWidget {
  ...
  const RegisterPage({super.key, required this.accountType, this.linkedUid});
  @override
...
class _RegisterPageState extends State<RegisterPage> {
  ...
  void validatePhone() {
    ...
    final phoneRegex = RegExp(r'^\+?[0-9]{9,19}$');
  ...
  Future<void> verifyPhone() async {
    ...
    if (phone.isEmpty || phoneError != null) return;

    try {
      final result = await showPhoneOtpDialog(context, phone: phone);
      if (result == null || !mounted) return;
      setState(() {
        isPhoneVerified = true;
        verifiedPhone = result.phoneNumber;
        verifiedPhoneCredential = result.credential;
        phoneError = null;
      });
    } on FormatException catch (error) {
      if (!mounted) return;
      setState(() => phoneError = error.message);
    }
  }
  ...
  Future<void> register() async {
    ...
    if (firstNameController.text.trim().isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context)!.requiredFields)),
      );
      return;
    }
    ...
    if (!kvkk || !terms || !privacy) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(AppLocalizations.of(context)!.acceptLegalDocuments),
        ),
      );
      return;
    }

    if (phoneController.text.trim().isNotEmpty && !isPhoneVerified) {
      setState(() => phoneError = 'Yazdığınız telefon numarasını doğrulayın.');
      return;
    }
    ...
    try {
      ...
      // İkinci profil oluşturuluyorsa, ilk hesabın kısa ömürlü oturum kanıtını
      // yeni hesap oluşturulmadan önce alırız. Bağlama sunucuda yapılır.
      String? sourceIdToken;
      if (widget.linkedUid != null && widget.linkedUid!.isNotEmpty) {
        final sourceUser = _auth.currentUser;
        if (sourceUser == null || sourceUser.uid != widget.linkedUid) {
          throw StateError('Bağlanacak hesap oturumu bulunamadı.');
        }
        sourceIdToken = await sourceUser.getIdToken();
        if (sourceIdToken == null || sourceIdToken.isEmpty) {
          throw StateError('Bağlanacak hesap doğrulanamadı.');
        }
      }
      ...
      if (emailExists || phoneExists) {
        setState(() {
          if (emailExists) {
            emailError = AppLocalizations.of(context)!.emailAlreadyExists;
          }

          if (phoneExists) {
            phoneError = AppLocalizations.of(context)!.phoneAlreadyExists;
          }
      ...
      if (!success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(AppLocalizations.of(context)!.verificationCodeFailed),
          ),
        );
      ...
      final verified = await Navigator.push<bool>(
        ...
        MaterialPageRoute(
          builder: (_) => VerificationCodePage(
            ...
            email: emailController.text.trim(),
            phone: isPhoneVerified ? verifiedPhone : '',
  ...
  Widget build(BuildContext context) {
    final hasEnteredPhone = phoneController.text.trim().isNotEmpty;
    final phoneVerificationRequired = hasEnteredPhone && !isPhoneVerified;
            ...
            children: [
              ...
              TextField(
                ...
                decoration: field(
                  ...
                  // Telefon gerçekten girildiğinde doğrulama seçeneğini göster.
                  // Boş bırakılan telefon üyeliği engellemez.
                  suffix: !hasEnteredPhone
                      ? null
                      : isPhoneVerified
                      ? const Icon(Icons.verified, color: Colors.green)
                      : TextButton(
                          onPressed: phoneError == null ? verifyPhone : null,
              ...
              TextField(
                ...
                onTap: () async {
                  ...
                  final result = await showSearch<String>(
                    context: context,
                    delegate: DistrictSearchDelegate(city: cityController.text),
                  );
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  // Kullanıcı telefon yazdıysa, SMS kodu ile doğrulayana kadar
                  // üyeliği başlatamaz. Telefon boşsa kayıt normal devam eder.
                  onPressed: isLoading || phoneVerificationRequired
                      ? null
                      : register,
